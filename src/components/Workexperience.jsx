...
const aviationExperience = [
  ...
  const ExperienceTimeline = ({ experiences, title }) => {
	return (
	  <div className="w-full max-w-2xl md:max-w-5xl mx-auto mb-8 md:mb-20 p-2 md:p-4 scale-75 md:scale-100 mt-2">
		<h3 className="text-xl md:text-3xl font-bold text-white mb-1 md:mb-12 text-center">{title}</h3>
		<div className="relative">
		  <div className="relative md:absolute left-1/2 transform -translate-x-1/2 w-0.5 h-full bg-indigo-700 hidden md:block"></div>
		  {experiences.map((exp, index) => (
			...
			>
			  ...
			  <div className={`w-full sm:w-full md:w-5/12 bg-transparent border-2 border-indigo-600 p-2 md:p-6 rounded-lg shadow-sm md:shadow-lg ${index % 2 === 0 ? 'text-center md:text-left md:pl-8' : 'text-center md:text-right md:pr-8'}`}>
				<p className="text-xs  sm:text-md md:font-semibold text-white">{exp.description}</p>
			  </div>
			</motion.div>
		  ))}
		</div>
...
const AviationWorkExperienceSection = () => {
	return (
		<Section>
		<div className="flex flex-col md:flex-row w-full items-start justify-start md:mt-12 md:pt-10 scale-80 md:scale-100">
		  <motion.div className="w-full md:w-3/4" whileInView={"visible"}>
			<h2 className="text-3xl md:text-5xl font-bold text-white mb-2 text-left">
			  Aviation Experience
			</h2>
			<div className=" md:space-y-16 w-3/4 md:w-full">
 ...
 const EnterpreunershipWorkExperienceSection = () => {
  return (
    <Section mobileTop={true}>
      <div className="flex flex-col md:flex-row w-full justify-start items-start md:mt-12 ">
        <motion.div className="w-full sm:w-3/4  items-start mb-2" whileInView="visible">
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-2 text-center">
            My Business Ventures
          </h2>
          <div className=" w-3/4 sm:w-full mt-2">
